...
export class BottleneckAnalyzer {
  ...
  static analyzeLibrary(library, systemInfo) {
    ...
    library.forEach(game => {
      ...
      analysis.compatibilityMatrix.push({
        gameId: compatibility.lookupKey || game.appid || game.app_id || game.steamAppId || game.name,
        gameName: gameName,
    ...
    analysis.compatibilityMatrix.sort((left, right) => {
      const levelOrder = { ultra: 4, high: 3, low: 2, cannot_run: 1, unknown: 0 };
      if (left.isEstimate !== right.isEstimate) {
        return Number(left.isEstimate) - Number(right.isEstimate);
      }
      if ((levelOrder[right.performanceLevel] || 0) !== (levelOrder[left.performanceLevel] || 0)) {
        return (levelOrder[right.performanceLevel] || 0) - (levelOrder[left.performanceLevel] || 0);
      }
    ...
    analysis.overallScore = this.calculateOverallScore(analysis.stats, analysis.stats.analyzedGames);
  ...
  static generateComponentRecommendation(component, gamesAffected, systemInfo, stats) {
    const recommendations = {
      cpu: {
        component: 'CPU',
